...
# 평균부하율 계산(D7, D8_M 공통)
  ...
  - MDM 44 0x2A2C Current RMS 		(서보 기존 기능임) 측정시간 설정시 동작함, 
  - MDM 45 0x2A2D Current RMS Max	0x2A2C의 최대값
# LMS 이동평균 부하율 시간 설정
  -LMS 이동평균 부하율 시간 설정(D7, D8_M 공통)
  - ft-1.55 0x2137 LMMT_Current Load Moving Avg Time Setting, 0(default, disable), 0~99999[sec]
  ----------- D8_M (A/B/C축) ------------
  - ft-3.44 0x232C Axis-A LMMT_Current Load Moving Avg Time Setting, 0~99999, 단위:sec
  - MDM 44 0x2A96	LMS Motor Utilization(RMS) 			: Valid 센서 ON동안의 Current Load Factor(RMS), 0x2417, 10(default), 0~60000 [ms]
  																		  (0x2A2C와 동일함, 토크 순시값)
  - MDM 45 0x2A97	LMS Motor Moving Utilization(RMS)	: Valid 센서 ON동안의 Current Load Factor(RMS)의 이동 평균값
  																		  (0x2137 설정시 이동평균값 표시)
...
# 속도 감속시점 조정 기능(베트남 하이퐁 이슈 대응)
   ...
   모드별 위치(12)/자동위치(52)/동시이동(62) 단계에 적용함
	...
	(D8)ft-1.54 (0:disable, 0 아닌 위치값을 입력시 등속에서 감속되는 시점을 임의 조정 가능)
