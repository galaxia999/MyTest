...
### 2) 동시이동 파렛 카운팅(pallet counter for moving) 설정 변수
	동시이동 파렛 카운팅 설정 변수
...
### 3) 새로 진입하는 파렛 카운팅 상태 변수 (Sync_MovePalletCount1, Sync_MovePalletCount2)
...
-------------------------------------------------------------------------------
동시이동 개발시 발생한 SIDE EFFECT, FW 버그
-------------------------------------------------------------------------------
...
### 4) 동시이동 구현 중 발생한 FW 오류들.
   #### 1) 위치명령 업데이트 오류로 비정상적인 구동 현상 수정
