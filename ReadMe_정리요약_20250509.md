...
### 출력 신호기능 : TG_ON, SRDY, P_COM, V_COM
	...
	LMS_IOMapping_SALM(); //MISO.2 SRDY
...
### 역주행 방지 로직1/2 추가, V1.00.99.34 :20230619
   ...
   >로직1: 스위칭동안 인코더 1주기 이전의 증분값을 적용하는 방식  
...
## LMMT 전용 일반 SPEC
-------------------------------------------------------------------------------  
### 파라메터 저장명령(0x1010:01,0x1010:04)시 파렛오프셋 자동저장 기능 추가
...
### 스텝이동
	...
	- target position+스텝 이동량-> 모터현재위치+스텝 이동량으로 설정함.
...
### 4) FW 비상정지 알람 (영우DSP 요청건)
   ...
   > 동작  
	...
	- 통신 SafeOP 변경시 비상정지 기능이 동작중이면 즉시 서보으프하지 않고 감속정지 하도록 개선함.  
...
### 3) 홈밍동작에서 홈밍오프셋 적용시 가감속 발생하지 않는 현상 개선
...
### 3) RFID 오프셋 탐색/적용 시점을 지연시키는 기능 추가(DCT서형식 수석 요청)
	- V01.20.10.84 20241206
	...
	* RFID 통신지연으로 인한 위치 오프셋 업데이트 늦어지는 것을 개선하기 위해 업데이트함수를 속도루프 주기로 갱신하도록 함.
...
### 파렛 이동을 위한 속도피드백 필터 옵션 적용 기능
	...
	> 속도명령 추정 예시
		- D8의 경우 1펄스는 속도 0.1m/s에 해당하고 구동속도가 0.5m/s이고 인코더가 +=20% 오차로 측정되었다면
		이 값을 3(=0.3m/s)이하로 설정한다.

### 속도제어를 위한 속도피드백 필터 옵션 적용 기능
	- ft-2.42 D1, 0x222A,  ft-2.42	Velocity Feedback Select for Velocity Loop
		0 : (속도피드백) raw data 
		1 : 1st LPF filtered
		2 : 2st LPF filtered
...
### 파렛 정지 상태를 판단하는 기능
	...
	- 이 범위내에서는 파렛 출발시 속도 0에서 출발하도록 만드는 기능이다.
	- ft-2.44,    0x222C,  ft-2.44	Encoder Delta Threshold for Motor Stop Detection
	> 속도이동시 속도명령 기준
...
-------------------------------------------------------------------------------
## 동시이동 개발시 발생한 SIDE EFFECT (FW 최적화 진행 중 발생된 버그를 수정했던 내용)
-------------------------------------------------------------------------------
### 1) 동시이동 구현하면서 위치틀어짐 발생하여 수정한 이력 사항 #1
	- V01.20.90.47 변경점 검토: 
...
### 2) 동시이동 구현하면서 위치틀어짐 발생하여 수정한 이력 사항 #2
