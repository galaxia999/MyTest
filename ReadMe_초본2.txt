...
# MISO.15 통신 비상정지 상태임을 알리는 비트 추가.

4) 필터링 처리 방식(D7 vs D8 비교해서 차이가 있음)
...
-------------------------------------------------------------------------------
DCT 인코더 양간오차 적용 SPEC
-------------------------------------------------------------------------------
1) DCT장비에서 엔코더 양간오차 보상 기능 로직
   -전원 ON/OFF시 혹은 알람 발생하고 알람 해제시 인코더 양간오차만큼 위치값 틀어지는 현상
   -홈밍했을 때의 위치와 달라서 이슈가 됨.
   -구현: Set_Update_PosFbk_with_EncDiff 함수 참고.
      1)EA > EB일때
         정방향(DIR= 1): 현재 위치값은 양간오차 절대값을 보정(-)한 값이므로 더하고 빼는 방식으로 보정함.
         역방향(DIR=-1): 현재 위치값은 양간오차 절대값을 보정(+)한 값이므로 빼하고 더하는 방식으로 보정함.
      2) EA < EB일때는 반대로함.
2)LMMT_PLS_Diff값이 HalfTurn 값 보다 큰경우 LMMT_Diff_Signed값 수정
   현재 위치 유지기능이 특정조건(A&B)구간에서 이상동작 현상 수정
   :DCT엔코더만 해당하고 RSA엔코더는 양간오차가 0 이므로 보정이 필요없다.
   
-------------------------------------------------------------------------------
DCT 인코더 파렛 오프셋 적용 SPEC
-------------------------------------------------------------------------------     
1) DCT장비에서 엔코더 파렛 오프셋 보상 기능 로직      
   이력: 진입 후 Valid A ON 될 때 적용되는 인코더 오프셋값이 인코더 전환 후 전환된 인코더의 정보가 사용되는 현상
   변경: 진입 후 Valid A&B 되는 시점에 적용되는 오프셋은 이전 엔코더 기준으로 적용됨.

   예시: (EncA보드 ID:3, EncB보드 ID:5 로 가정하고 설명함)
      1) polarity : Normal일때
        1-1)정방향 이동
...
LMMT 전용 일반 SPEC
-------------------------------------------------------------------------------  
# 파라메터 저장명령(0x1010:01,0x1010:04)시 파렛오프셋 자동저장 기능 추가(DCT요청사항, D7과 기능동작이 동일하도록 통일함), V1.00.99.42 :20231012
...
# LMS_TorqueLimit_readyFlag(토크제한 플래그) 동작 정의
	0x2A98	152 LMS_Torque Limit Ready
	토크제한 플래그(0x2A98 LMS_TorqueLimit_readyFlag)를 Status Word 비트14 에 할당함.
...
2) 인코더 오류 알람
   E.031 ENCPE, 인코더 무효 : 엔코터 위치값 무효인 경우 알람 발생 추가 (DCT special)
   E.112 ESTOP (DCT special) : ft-1.51 D0 Enable일때 근접센서 조건으로 발생함
   
3) HW 비상 정지 개선, V01.20.10.87 20250114, V1.00.99.62 20250115
   -HW 비상정지시 감속 속도 시작은 속도명령을 기준으로 한다. (현재 속도를 적용하면 리플이 많아서 가감속될 수 있음)
   -HW 비상정지시 감속값(ft-2.07)이 로터리모터 기준(rev/s^2)에서 리니어 단위(mm/s^2)로 적용되도록 수정함.
...
5) 동시이동 구현시 발생한 side effect, V1.00.99.66 20250214 
   #위치명령 업데이트 오류로 비정상적인 구동 현상 수정
      -홈밍 기능 수행시 홈밍 후 정지위치가 홈오프셋으로 동작하지 않고 제1/2위치값으로 동작하는 현상
   ...
   # 위치정지 + GO OFF 상태에서 구동방향(MOSI.3) 바꿀 때 파렛이 이동하는 현상
      -59버전에서 수정한 Update_TargetPositionWithDirection 함수를 LMMT control에서 항상 호출하도록 변경한 부분의 side effect
...
-------------------------------------------------------------------------------
1) 실시간 위치 override 기능 (DCT 김희철 전임 요청사항)
   ...
   -속도.가감속 PDO 맵핑 기능 추가 (0x2123~0x2128, 각축), 0) V1.00.99.42 :20231012
2) S커브 실시간 적용되도록 동작 개선
...
# 자동위치 모드 개선사항 : V01.20.10.86 20241231
...
2) A/B/C축 슬롯에 대한 ModuleIdent 개별할당
  	Axis-A : x619800
  	Axis-B : x719800
  	Axis-C : x819800
3) PDO 업데이트 방식 : DC sync 인터럽트에서 동작 (검색어: PDO_UPDATE_ISR)
4) EtherCAT 0x6060(Modes of operation) 동작 모드와의 관계
...
2) 동시이동 사용시 위치값 갱신하는 방법 결정, V1.00.99.64 20250207
  -D7: 송신ID(TxID) 기준
...
3) RFID 오프셋 탐색/적용 시점을 지연시키는 기능 추가
   V01.20.10.88 20250122 
	-동시이동시 DCT 인코더의 처리 시간 부족으로 RFID값을 전송하지 못함.
	-인코더에서 RFID 처리하는데 최대 400ms 정도의 시간이 필요하기 때문 RFID 받은 수신타이밍을 늦게 적용하는 기능으로 개발됨.
	동시이동 기능 사용시 엔코더 스위칭후 400ms 시간지연 후 RFID를 이용한 파렛오프셋 기능이 동작하도록 설정함.
	RFID Offset Apply Delay Time							 	0x2232		Ft-2.50 	0x2232	P0-2.50		Y	6018
...
# 무버 ID 동작 사양(10.61기준), 남기혁책임 요청사항
...
# 동시이동 파렛 카운팅(pallet counter for moving) 설정 변수
	동시이동 파렛 카운팅 설정 변수
	ft-2.47 [D0..D3] Pallet Out Counter for SyncMotion
...
# 새로 진입하는 파렛 카운팅 상태 변수 (Sync_MovePalletCount1, Sync_MovePalletCount2)
...
# 속도피드백 필터 적용 옵션 추가 (1차, 2차 LPF, MAF, Velocity Cmd Follow)
   ft-1.17=1일때만 적용, MAF_Init/MAF_Create/MAF_Update

# 속도피드백 평균횟수 변경 : 4->8 ((DCT요청사항), V1.00.99.33 :20230420 
# 동작중에도 속도피드백 필터를 변경할 수 있도록 함. V1.00.99.38 :20230630
...
1) 위치모드에서 등속 이동중 운전 속도 변경되도록 수정. (DCT 김혜찬 전임 요구사항), V1.00.99.24 :20221207
   -Update_velocityAccDec 함수
   
2) 현재 위치값이 위치 티칭값과 같은 값으로 보이도록 수정. (DCT요청사항 고정도)V1.00.99.25 :20230106
   -이유: 파렛오프셋 적용된 값을 상위로 보여지면 운영자는 위치완료를 판단하지 못하기 때문.
     타켓위치(0x607A)=위치 티칭값
     현재 위치 내부(0x6063)=현재 위치값(티칭값 기준) + 파렛오프셋값
     현재 위치(0x6064)=현재 위치값(티칭값 기준)
...
5)위치정지 상태 + GO OFF 상태에서 위치값 변경시 속도명령 발생하여 파렛이 움직이는 현상 수정(D7 V1.20.10.81 대응)
   V01.20.10.81 20240903
   -위치정지 상태에서 위치값만 변경시 속도명령 발생하여 파렛이 움직이는 현상 수정 
	증상: 백호프 제어기 사용시 GO와 위치명령을 동시에 변경하여 진행하였음. 이와 같은 상황에서는 이슈가 발생하지 않았음												
	원인:	
...
6) 위치정지 + GO OFF 상태에서 구동방향(MOSI.3) 바꿀 때 파렛이 이동하는 현상
   V01.20.10.90 20250312
  -81버전에서 수정한 Update_TargetPositionWithDirection 함수를 LMMT control에서 항상 호출하도록 변경한 부분의 side effect
    원인: 정지 상태에서 방향을 바꾸면 Update_TargetPositionWithDirection 함수에 의해서 target position이 변경된다.
    이때 LMS_STATUS는 변경되지 않은 상태에서 위치에러(pos_err)만 발생하여 속도명령이 생성되어 파렛이 이동하게 됨.
    수정: 정지상태에서 방향을 바꾸면 LMS_STATUS(10) 준비상태로 천이함.
    정지상태에서 위치값을 바꾸면 LMS_STATUS(10) 준비상태로 천이하는 것과 동일하게 처리함.
  Update_Direction_And_StatusChanging(ax) 추가하고 Update_TargetPosition_with_PDOupdateTiming 함수와 함께 사용함.
  -적용되는 상태: 위치완료(13/53/63) 상태에서 구동방향 변경시 위치준비(10)

6) 위치/자동위치/동시모드의 위치정지 상태에서의 동작 사양, V1.00.99.60 20241231
	(1)위치모드 진입시  MOSI.2 OFF시 현재 위치 유지
	(2)자동위치모드 진입시  MOSI.8, MOSI.2 동시 OFF시 현재 위치 유지(53)
	(3)동시이동모드 진입시  MOSI.9, MOSI.2 동시 OFF시 현재 위치 유지(63)

1) 위치 정지시 영속도 제어로 정지하는 기능 확대 적용, V1.00.99.61 20250110
...
3) 스텝모드에서 기동시 속도명령 변경에 따라 실시간 가감속 업데이트되도록 동작 개선
     ...
     대기(READY) 상태에서 위치값(제1/2 위치값) 변경시 적용되도록 수정
     대기(READY) 상태에서 기동시 속도명령(1/2 Velocity) 변경시 가감속값 업데이트함.	


3) 위치 정지시 0속도 제어로 정지하도록 개선함 V1.00.99.35 :20230621
   -서보온상태에서 파렛이 진입할 때  위치 정지명령이면 프리런되는 현상 개선    
   

4) 위치 정지시 영속도 제어로 정지하는 기능
   V01.20.10.87 20250114 
   -서보온상태에서 파렛이 진입할 때 위치 정지 명령이면 프리런되는 현상 개선
   -외부에서 Servo On -> Pos Mode -> 파렛 진입 시 Free Run 되는 현상 발생함
...
# 속도 감속시점 조정 기능(베트남 하이퐁 이슈 대응)
   ...
   - D7 : ft-1.44 (0:disable, 0 아닌 위치값을 입력시 등속에서 감속되는 시점을 임의 조정 가능)
   - D8 : ft-1.54
   모드별 위치(12)/자동위치(52)/동시이동(62) 단계에 적용함
   - 속도 프로파일 생성기는 목표위치에서 위치스위칭 임계값 (position switching threshold)을 뺀 가상의 위치를 
   기준으로 주어진 속도와 감속시간을 고려하여 감속 시점을 결정한다.
   - 최종 목표위치 대비 남은 위치가 이 임계값보다 작을 때 위치모드(13) 정지단계로 전환된다.
...
-------------------------------------------------------------------------------
LMMT 전용 FW 작성시 유의 사항
-------------------------------------------------------------------------------  
1)인코더 양간오차 발생시
	-인코더 전환시점에 발생하는 오프셋만큼을 위치편차로 계산한다.
	-위치값 갱신시 오프셋 처리를 항상 고민할것.
	
2)모터 정지시 떨림으로 시작하는 속도명령이 0에서 출발하지 않는 현상
...
-------------------------------------------------------------------------------
LMMT 전용 FW 추가, 개선할 내용들, to do list
-------------------------------------------------------------------------------  
Known Isses.
9) A/B 개별 불감대 영역 설정 기능(1.50은 테스트용으로 추가 변경예정, )
...
# 인코더 스위칭시 Direct(off), Tranf(on)되는 특수한 경우에 엔코더값이 0 으로 초기화되는 현상을 개선함
...
LMMT 전용 FW 버그 수정
-------------------------------------------------------------------------------  
1) 동시이동 위치정지 후 위치티칭값을 변경할때 속도이동되는 버그 수정, 0) V01.20.10.43->V01.20.10.44 20230919
...
///////////////////////////////////////////////////////////////////////////////
5) RSWare OscilloScope Trigger 안되는 현상 수정
